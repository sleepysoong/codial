...
from codial_discord.app.core_api_client import CoreApiClient
from codial_discord.app.discord_api_client import DiscordApiClient
from codial_discord.app.security import verify_discord_request
from codial_discord.app.session_store import store
from codial_discord.app.settings import settings
from libs.common.logging import get_logger

router = APIRouter()
logger = get_logger("codial_discord.routes")
...
def _extract_option_int(data: dict[str, Any], option_name: str) -> int | None:
    value = _extract_option_value(data, option_name)
...
async def _set_subagent_from_command(payload: dict[str, Any]) -> None:
    async with _job_semaphore:
        channel_id = payload.get("channel_id", "")
        data = payload.get("data", {})
        subagent_input = _extract_option_string(data, "name") if isinstance(data, dict) else ""
        normalized_input = subagent_input.strip()
        normalized_name = None if normalized_input in {"", "none", "해제"} else normalized_input

        if not isinstance(channel_id, str) or not channel_id:
            logger.error("missing_channel_id_for_subagent")
            return

        binding = await store.get_by_channel_id(channel_id)
        if binding is None:
            logger.warning("subagent_update_dropped_channel_not_bound", channel_id=channel_id)
            return
        ...
        try:
            config = await core_client.set_subagent(binding.session_id, normalized_name)
            active_subagent = config.get("subagent_name")
            if isinstance(active_subagent, str) and active_subagent:
                message = f"서브에이전트를 `{active_subagent}`로 설정했어요."
            else:
                message = "서브에이전트 설정을 해제했어요."
            ...
            logger.exception("subagent_update_failed", session_id=binding.session_id, error=str(exc))
            await discord_client.create_channel_message(
                channel_id=channel_id,
                content="서브에이전트 설정에 실패했어요. 이름을 확인하고 다시 시도해요.",
            )
...
def _format_codial_rule_list(rules: list[str]) -> str:
    if not rules:
        return "CODIAL 규칙이 아직 없어요. `/규칙추가`로 새 규칙을 추가해요."
    lines = [f"{index}. {rule}" for index, rule in enumerate(rules, start=1)]
    return "현재 CODIAL 규칙 목록이에요.\n" + "\n".join(lines)


async def _list_codial_rules_from_command(payload: dict[str, Any]) -> None:
    async with _job_semaphore:
        channel_id = payload.get("channel_id", "")
        if not isinstance(channel_id, str) or not channel_id:
            logger.error("missing_channel_id_for_codial_rules_list")
            return
        ...
        try:
            result = await core_client.get_codial_rules()
            rules_value = result.get("rules")
            rules = [item for item in rules_value if isinstance(item, str)] if isinstance(rules_value, list) else []
            await discord_client.create_channel_message(
                channel_id=channel_id,
                content=_format_codial_rule_list(rules),
            )
        except Exception as exc:
            logger.exception("codial_rules_list_failed", channel_id=channel_id, error=str(exc))
            await discord_client.create_channel_message(
                channel_id=channel_id,
                content="규칙 목록을 불러오지 못했어요. 잠시 뒤에 다시 시도해요.",
            )


async def _add_codial_rule_from_command(payload: dict[str, Any]) -> None:
    async with _job_semaphore:
        channel_id = payload.get("channel_id", "")
        data = payload.get("data", {})
        rule = _extract_option_string(data, "rule") if isinstance(data, dict) else ""

        if not isinstance(channel_id, str) or not channel_id:
            logger.error("missing_channel_id_for_codial_rules_add")
            return
        if not rule.strip():
            logger.error("missing_rule_option", channel_id=channel_id)
            return
        ...
        try:
            result = await core_client.add_codial_rule(rule.strip())
            rules_value = result.get("rules")
            rules_count = len(rules_value) if isinstance(rules_value, list) else 0
            await discord_client.create_channel_message(
                channel_id=channel_id,
                content=f"규칙을 추가했어요. 현재 규칙 수는 {rules_count}개예요.",
            )
        except Exception as exc:
            logger.exception("codial_rules_add_failed", channel_id=channel_id, error=str(exc))
            await discord_client.create_channel_message(
                channel_id=channel_id,
                content="규칙 추가에 실패했어요. 입력값과 서버 상태를 확인해요.",
            )


async def _remove_codial_rule_from_command(payload: dict[str, Any]) -> None:
    async with _job_semaphore:
        channel_id = payload.get("channel_id", "")
        data = payload.get("data", {})
        index = _extract_option_int(data, "index") if isinstance(data, dict) else None

        if not isinstance(channel_id, str) or not channel_id:
            logger.error("missing_channel_id_for_codial_rules_remove")
            return
        if index is None or index < 1:
            logger.error("invalid_rule_index_option", channel_id=channel_id, index=index)
            return
        ...
        try:
            result = await core_client.remove_codial_rule(index)
            rules_value = result.get("rules")
            rules_count = len(rules_value) if isinstance(rules_value, list) else 0
            await discord_client.create_channel_message(
                channel_id=channel_id,
                content=f"규칙 {index}번을 제거했어요. 현재 규칙 수는 {rules_count}개예요.",
            )
        except Exception as exc:
            logger.exception("codial_rules_remove_failed", channel_id=channel_id, index=index, error=str(exc))
            await discord_client.create_channel_message(
                channel_id=channel_id,
                content="규칙 제거에 실패했어요. 번호를 확인하고 다시 시도해요.",
            )
...
async def discord_interactions(
    ...
    if interaction_type == 2:
        ...
        if command_name == "mcp":
            ...
            return {"type": 5, "data": {"flags": 64}}
        if command_name in {"subagent", "서브에이전트"}:
            _schedule_background_job(
                _set_subagent_from_command(payload),
                job_name="set_subagent_from_command",
            )
            return {"type": 5, "data": {"flags": 64}}
        if command_name in {"규칙목록", "rules_list"}:
            _schedule_background_job(
                _list_codial_rules_from_command(payload),
                job_name="list_codial_rules_from_command",
            )
            return {"type": 5, "data": {"flags": 64}}
        if command_name in {"규칙추가", "rules_add"}:
            _schedule_background_job(
                _add_codial_rule_from_command(payload),
                job_name="add_codial_rule_from_command",
            )
            return {"type": 5, "data": {"flags": 64}}
        if command_name in {"규칙제거", "rules_remove"}:
            _schedule_background_job(
                _remove_codial_rule_from_command(payload),
                job_name="remove_codial_rule_from_command",
            )
            return {"type": 5, "data": {"flags": 64}}
